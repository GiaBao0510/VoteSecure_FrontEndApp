    ...
    methods: {
        ...
        async Update(data) {
            try {
                ...
                const response = await api.put(`${import.meta.env.VITE_CADRE_API}/${this.entity.iD_CanBo}`, {
                    HoTen: data.hoTen,
        ...
        // Xóa
        async Delete(iD_CanBo) {
            try {
                console.log("Xóa: ", iD_CanBo);
                console.log("Api: ", `${import.meta.env.VITE_CADRE_API}/${iD_CanBo}`);
                ...
                if (result.isConfirmed) {
                    const response = await api.delete(`${import.meta.env.VITE_CADRE_API}/${iD_CanBo}`);
                    if (response.status === 200) {
                        Swal.fire({
                            icon: 'success',
        ...
        async handleImageChange(event) {
            ...
            try {
                ...
                const response = await api.put(
                    `${import.meta.env.VITE_CADRE_API}/update-image/${this.entityLocal.iD_CanBo}`,
                    formData,
                    {
                        headers: {
