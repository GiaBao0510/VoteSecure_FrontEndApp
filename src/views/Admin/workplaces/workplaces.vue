            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="DataModalLabel">Thêm hoạt động mới</h5>
...
import AddForm from '../../../components/workplaces/AddWorkplace.vue';
import Loading from '../../Loading.vue';
import { Modal } from 'bootstrap';
import DetailedForm from '../../../components/workplaces/workplaceDetails.vue';
...
export default {
    ...
    methods: {
        ...
        async fetchDatas() {
            this.isLoading = true;
            try {
                const res = await api.get(import.meta.env.VITE_WORKPLACE_API);
                if (res.status === 200) {
                    // Chuyển đổi 'T' thành khoảng trắng trong chuỗi ngày tháng
                    this.rows = res.data.data.map(item => ({
                        ...item,
                        ngayBD: item.ngayBD.replace('T', ' ')
                    }));
